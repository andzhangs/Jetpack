package zs.android.jetpack.service
...
import zs.android.jetpack.BuildConfig
import zs.android.jetpack.R
import zs.android.jetpack.receiver.NotificationsReceiver
...
class CompressService : Service() {
    ...
    private val CHANNEL_ID by lazy { "100_${DownloadService::class.java.canonicalName}" }
    private val CHANNEL_NAME ="压缩进度通知"
...
//                importance = NotificationManager.IMPORTANCE_HIGH  //重要性（Android 8.0 及更高版本）
        ...
        val cancelPendingIntent =
            Intent(this, NotificationsReceiver::class.java).let { intent ->
                intent.action = NotificationsReceiver.ACTION_CANCEL
                intent.putExtra(NotificationsReceiver.KEY_NOTIFY_ID, NOTIFY_ID)
                intent.putExtra(NotificationsReceiver.KEY_NOTIFY_MESSAGE, "I am from ${this.javaClass.canonicalName}")
...
//            .setLargeIcon(BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher_foreground))
            .setContentTitle("压缩完成：100个") ///$mProgressMax

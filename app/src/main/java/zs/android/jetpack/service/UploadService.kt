...
class UploadService : Service() {
    ...
    private val CHANNEL_ID by lazy { "300_${UploadService::class.java.canonicalName}" }
    private val CHANNEL_NAME = "上传通知"
    ...
    private fun createNotification() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                //设置渠道描述
                description = "上传 media [image or video]"
                //设置提示音
//                setSound()
                //开启指示灯
                enableLights(false)
...
//                importance = NotificationManager.IMPORTANCE_HIGH  //重要性（Android 8.0 及更高版本）
        ...
        val cancelPendingIntent =
            Intent(this, NotificationsReceiver::class.java).let { intent ->
                ...
                intent.putExtra(
                    NotificationsReceiver.KEY_NOTIFY_MESSAGE,
...
//            .setLargeIcon(BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher_foreground))
            .setContentTitle("上传完成：10个") ///$mProgressMax
